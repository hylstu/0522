	(function($) {
		...
		$.suggest = function(input, options) {
			...
			var timeout = false;		// 暂停
			var prevLength = 0;			// 记录$input.val()最后的长度
			var cache = [];				// 缓存列表
			...
			resetPosition();
			$(window)
				.load(resetPosition)		// 用户在加载时更改页面大小
			...
			function displayItems(items) {
				...
				if (items=='') {//热门城市遍历
					for(h in options.hot_list){
						html+='<li rel="'+options.hot_list[h][0]+'"><a href="Merchant_recommend.html'+h+'"><span>'+options.hot_list[h][2]+'</span>'+options.hot_list[h][1]+'</a></li>';
					}
					...
					for (var i = 0; i < options.source.length; i++) {//国内城市匹配
						...
						if (reg.test(options.source[i][0]) || reg.test(options.source[i][1]) || reg.test(options.source[i][2]) || reg.test(options.source[i][3])) {
							html += '<li rel="' + options.source[i][0] + '"><a href="Merchant_recommend.html' + i + '"><span>' + options.source[i][2] + '</span>' + options.source[i][1] + '</a></li>';
						}
					}
					if (html == '') {
