/*    var dlNum  =$("#selectList").find("dl");
    //for (i = 0; i < dlNum.length; i++) {
     //   $(".Page_Con_tits .clearList").append("<div class=\"selectedInfor selectedShow\" style=\"display:none\"><span></span><label></label><em></em></div>");
   // }
...
//         $(".Page_Con_tits .clearList").append(infor);
...
var oClearList = $(".Page_Con_tits .clearList");
...
oSelectList.onclick = function(e, a) {
	...
	var aRadios = document.getElementsByName('radio1');
    if( isCusPrice ) {
      //radioVal = oCustext1.value + '-' + oCustext2.value + '元';
	 radioVal = oCustext1.value + '-' + oCustext2.value ;
    } else {
      radioVal = '';
    }
...
function delStr(str, arr) { //删除数组给定相同的字符串
    ...
    for (var i = 0,len = arr.length; i < len; i++) {
			//alert("aaa"+str+"wwwwwwwwww"+arr[i]+"ccc");
			if (str==arr[i]) {
